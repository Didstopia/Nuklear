...
\returns Returns 0 on success, -1 on failure.
*/
int rectangleColor(SDL_Renderer *renderer, Sint16 x1, Sint16 y1, Sint16 x2, Sint16 y2,
                   Uint32 color) {
  Uint8 *c = (Uint8 *)&color;
  ...
  */
  ...
  do {
    ...
    if (oct == startoct) {
      ...
      */
      if (oct % 2)
        drawoct |= (1 << oct); /* this is basically like saying drawoct[oct] = true, if drawoct were
                                  a bool array */
      else
    ...
    if (oct == endoct) {
      ...
      if (startoct == endoct) {
        ...
        /* otherwise: we only draw in this octant, so initialize it to false, it will get set back
         * to true */
        if (start > end) {
  ...
  /* so now we have what octants to draw and when to draw them. all that's left is the actual raster
   * code. */
...
\returns Returns 0 on success, -1 on failure.
*/
int aapolygonColor(SDL_Renderer *renderer, const Sint16 *vx, const Sint16 *vy, int n,
                   Uint32 color) {
  Uint8 *c = (Uint8 *)&color;
